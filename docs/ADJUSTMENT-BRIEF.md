...
| 14 | 2026-09-15 | **The 432A substitution formula must be divided by Calibration Factor on this bench, not Effective Efficiency.** The 432A manual prints the equation (3-30h) with EFFECTIVE EFFICIENCY under the line, because the precision procedure it belongs to assumes a tuner. The 478A's own manual is explicit about which applies when: Calibration Factor is used for "all measurements made without a tuner" (paragraph 33), Effective Efficiency "when a tuner is used to match the thermistor mount to the transmission line" (paragraph 35). There is no tuner here. Effective Efficiency is always the larger of the two, so following the printed formula would report **every** thermistor power low by a couple of percent - small enough to look like a legitimate disagreement between sensors rather than a systematic error. `Hp432A.Correction` defaults to Calibration Factor and the reading records which was used. |
| 15 | 2026-09-15 | **Three of the 432A's seven range legends are rounded.** Table 1-1 gives full scales of "10, 30, 100, and 300 uW, 1, 3 and 10 mW" *and* "-20 dBm to +10 dBm full scale in 5-dB steps". Those disagree: exact 5 dB steps from -20 dBm are 10, **31.62**, 100, **316.2**, 1000, **3162** and 10000 uW. The dBm figures are the exact ones - the service section corroborates it, giving the range amplifier a gain "variable from 1 to 31.6", which is 30 dB across seven ranges. Scaling the recorder output by the printed milliwatt legend would put a 5% (0.23 dB) error into three of the seven ranges, larger than the meter's own 1%-of-full-scale accuracy. `ThermistorRange` is defined by dBm and derives watts. |
| 16 | 2026-09-15 | **The 50 Ω feedthrough must not go between the crystal detector and the scope.** GEAR-MAP previously treated the DS1104Z's 1 MΩ input as a shortcoming to be corrected with a 10100C. The 8470B's manual says otherwise: output impedance is 1–2 kΩ (typically 1.3 kΩ), the sensitivity specification carries footnote 5 "External load resistance > 50 kΩ", and the square-law specification needs "> 8 kΩ". A 50 Ω load costs **28.6 dB**, voids both specifications, and puts the bottom of the useful range below the detector's own 50 µVpp noise — at −30 dBm the loaded output is 18 µV. The 1 MΩ input is the right one. `DetectorCalibrator.CheckSetup` warns with the number, and a calibration records the load it was taken with. **The 8473C's own manual is not in the library**, so these are the 8470B's figures carried across as same-family estimates and flagged as such. |
